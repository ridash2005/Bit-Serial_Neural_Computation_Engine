...
module tb_relu_activation;
    ...
    logic                     in_valid;
    logic                     out_ready;  // ADDED: Required by DUT
    ...
    relu_activation #(
        ...
        .in_data  (in_data),
        .in_valid (in_valid),
        .out_ready(out_ready),  // ADDED: Connect out_ready
        .out_data (out_data),
        .out_valid(out_valid)
    );
    ...
    // FIXED: Only pop when handshake completes (out_valid && out_ready)
    always @(posedge clk) begin
        if (rst_n && out_valid && out_ready) begin  // FIXED: Added out_ready check
